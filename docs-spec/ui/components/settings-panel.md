...
- 音声の設定（Devices タブ）は `settings_get` で読み、変更は 1 つずつ `settings_change` に渡す。保存・接続中のセッションへの反映・値の検査はバックエンド（`src-tauri/src/settings.rs`）が行い、Adapter は返ってきた「変更後の設定」をそのまま表示する。選択肢（デバイス・使っているデバイスのチャンネル数・バッファサイズ・サンプルレート）もバックエンドが返したものを出す（ADR-043、REQ-GUI-024）。デバイスがチャンネル数を申告しないときは、いま選んでいる組が見える数（少なくとも 2）だけ出す
