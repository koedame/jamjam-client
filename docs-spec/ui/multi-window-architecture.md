...
（真の per-peer 値ではない）。同様にフェーダー・パン・出力メーターも peer ID を取らない
単一のグローバルバックエンド API（`streaming_set_peer_volume`/`_pan`）へroutingされるため、
3人以上のセッションでは任意の参加者のフェーダー操作が全参加者の音声に等しく影響する。
1:1セッション（想定する主用途）では実害がないが、per-peer 制御にはバックエンドAPIの
拡張（peer_id引数の追加）が必要で、本PRのスコープ外の別タスクとする。
