...
**受信経路は GUI と同じ**: 受信した音声はコアライブラリの `ReceivePath`（`audio/stream.rs`）を通る。デコード・サンプルレート変換・プレイアウトバッファ・PLC・FEC は GUI と同一のコードであり、ジッタや損失に関わる問題は CLI でも GUI と同じ音で再現する。送信も GUI と同じくステレオ（センター）・プリセットのコーデックと FEC で行う。`/stats` と終了時の統計には、プレイアウトバッファで再生・補間・破棄されたフレーム数が出る。
