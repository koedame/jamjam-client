...
Feature: 音声品質
  ...
  @REQ-AUD-121 @must
  Scenario: 左右に同じチャンネルを選んだ入力は、モノラルとして取り込む
    Given 入力チャンネルの左右に同じ番号を選んでいる（または右を選んでいない）
    When 送信チャンネルを「ステレオ」にして接続する
    Then 入力から1チャンネルを取り込み、相手にもモノラルと伝える
    # 同じ音を2つ取り込んで「ステレオ」と偽らない。デバイスが1チャンネルしか持たないときも同じ

  @REQ-AUD-122 @must
  Scenario: デバイスに合わせた入出力チャンネルの当てはめは、デバイスを開くときだけに行い、保存した組を書き換えない
    Given 入力チャンネルを「5」と「6」に設定している
    When 入力デバイスを、その番号を持たないデバイスに切り替える
    Then そのデバイスの先頭のチャンネルが取り込まれるが、保存した設定は「5」と「6」のままである
    And 元のインターフェースに戻すと、「5」と「6」が取り込まれる
    # 設定画面には、いま実際に使われているチャンネルを表示する

  @REQ-AUD-123 @must
  Scenario: デバイスのドライバが固まっても、設定の読み書きと別のデバイスへの切り替えは待たされない
    Given 接続中に、入力デバイスのドライバが応答しなくなる
    When 設定を読む、別の設定を変える、または別のデバイスを選び直す
    Then どれも、固まった呼び出しが戻るのを待たずに反映される
    And デバイスを開く待ちには上限があり、超えると画面に「デバイスが応答しない」と出る
    And 利用状況の送信も、デバイスの読み出しを待たずにアプリの他の処理を止めない
    And 診断タブも、デバイスの読み出しを待たずにアプリの他の処理を止め、応答しない側を「デバイスが応答しない」問題として出す
    # 待ちが続くと、同じスレッドが運ぶネットワークも止まり、音声の接続が切れる

  @REQ-AUD-124 @must
  Scenario: 入力デバイスがセッションのサンプルレートで開けないときは、デバイスが対応するレートで開いて、アプリ側でセッションのレートに変換する
    Given セッションのサンプルレートが「48000 Hz」で、入力デバイスが「44100 Hz」でしか開けない（Windows のサウンド設定の既定の形式が 44100 Hz）
    When セッションを始める
    Then 入力は 44100 Hz で開き、48000 Hz に変換して送られる
    And 変換が足す遅延は 0.3ms 未満で、10kHz までの音量の変化は 0.1dB 以内である
    # 変換はデバイスのコールバックの中で、メモリを確保せずに行う。デバイスがセッションのレートで開けるときは変換しない

  @REQ-AUD-125 @must
  Scenario: 入力デバイスがどのレートでも開けなくても、セッションは続き、相手の音は聞こえ、画面に直し方が出る
    Given 入力デバイスが開けない
    When セッションを始める
    Then 出力は開いたままで、相手の音が聞こえる
    And 画面に「入力デバイス（名前）を 48000Hz で開けません。サウンド設定で、このデバイスの形式を 48000Hz にしてください」と出る
    # 形式が合わないこと以外で開けない場合は「入力デバイスを開けませんでした（名前）」と出る
