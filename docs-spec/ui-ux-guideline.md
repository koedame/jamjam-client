...
> **正**: このセクションの値は [design-tokens.md](./ui/design-tokens.md) の要約。差異が
> 生じた場合は design-tokens.md を正とする。

### 8.1 ダークモード（デフォルト）

| トークン | 用途 | 値 |
|---------|------|-----|
| `--color-bg-primary` | 画面背景（`bg-page`） | #1C1C1E |
| `--color-bg-secondary` | カード背景（`bg-card`） | #2C2C2E |
| `--color-bg-tertiary` | 行・入力欄背景（`bg-row`） | #141414 |
| `--color-bg-elevated` | ホバー時の浮き上がり | #48484A |
| `--color-accent` | 主要アクション、フォーカス（`accent-blue`） | #0A84FF |
| `--color-accent-hover` | アクセント（ホバー） | #3D9FFF |
| `--color-brand` | ブランド（ロゴ専用、`yellow-primary`） | #FFD60A |
| `--color-success` | 成功（`color-meter-low`） | #30D158 |
| `--color-warning` | 警告（`color-meter-mid`） | #FFD60A |
| `--color-danger` | エラー（`color-meter-high`） | #FF453A |
| `--color-text-primary` | テキスト（主、`text-light`） | #F5F5F7 |
| `--color-text-secondary` | テキスト（副、`gray-400`） | #98989D |
| `--color-text-disabled` | テキスト（無効、`gray-500`） | #6E6E73 |
| `--color-border` | ボーダー（`border-dark`） | #38383A |
| `--color-border-focus` | フォーカスボーダー | #0A84FF |

### 8.2 ライトモード

ui.pen にライトモード仕様が存在しないため、ダークモードと同一の値を暫定使用する
（[design-tokens.md](./ui/design-tokens.md#ライトテーマ) 参照）。
