...
| `mixer_get` | ミキサーのフェーダーの位置を読む |
| `mixer_set_local_volume` | 自分のマイクの音量を変える |
| `mixer_set_local_pan` | 自分のマイクの左右の位置を変える |
| `mixer_set_peer_volume` | 相手ごとの音量を変える |
| `mixer_set_peer_pan` | 相手ごとの左右の位置を変える |
| `mixer_set_peer_muted` | 相手ごとの音を消す・戻す |
