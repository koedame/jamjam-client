...
- 停止→再開始の操作は不要
- デバイスを開く処理は、セッションのスレッド（ネットワークも運ぶ）の外で行う。別のデバイスを選び直すと、開いている途中のものは見捨てて、すぐ新しい方を開く（[ADR-050](../adr/ADR-050-device-calls-are-bounded.md)）
...
ドライバが応答しなくなると、デバイスを開く呼び出し・一覧を読む呼び出しが戻らなくなる（macOS では `coreaudiod` や、インターフェースのドライバで起きる）。

- **上限**: ストリームを開くのは 8 秒（`OPEN_TIMEOUT`）、デバイスの一覧を読むのは 3 秒（`LIST_TIMEOUT`）。超えたら `AudioError::DeviceUnresponsive` を返す。呼び出しそのものは止められないので、専用のスレッドに残し、戻ってきても結果は捨てる（開いてしまったストリームは閉じる）
- **設定の読み書きは待たされない**: 一覧が固まっている間は、最後に読めた一覧を出す。固まった呼び出しが戻るまで、同じ一覧を読む呼び出しは重ねない
- **画面に出す**: 開けなかったデバイスは `streaming_status` の `device_problems` に、向き（`input` / `output`）と理由（`unresponsive` / `failed`）、分かればデバイス名を載せる。同じ向きのデバイスが開けたとき、セッションが始まるとき・終わるときに消える
...
├── device.rs       # デバイス管理
├── driver.rs       # 戻らないかもしれないドライバ呼び出しの上限（ADR-050）
