...
| REQ-RMT-002 | 手伝いのあいだ、手伝う人の手元に、手伝われる人のアプリと同じ画面が別のウィンドウで開く。この画面は、自分のアプリの画面と同じコードを、自分のアプリの状態ではなく、手伝われる人のアプリに繋いだ口の上で描く。手元の操作は手伝われる人のアプリに 1 件ずつ届き、そのアプリが手伝いの口の許可の表で判定して実行する。操作のたびに許可は聞かない。手伝われる人のアプリの状態（部屋・参加者・ミュート・音声の設定）の変化は手伝う人の画面に反映され、手伝う人が手元でするミュートと音声の設定の変更は、手伝われる人のアプリとその画面に反映される。手伝う人の画面には、相手の代わりに話す操作・退室する操作・相手の履歴や接続先を読む操作を出さない | must |
...
| REQ-RMT-008 | 2 台のアプリを同じルームに入れたとき、手伝う側の申し出・手伝われる側の許可（初めの 1 回だけ）・手伝う側のウィンドウが手伝われる側の画面を描くこと・手元のミュートと音声の設定の変更が相手のアプリに反映されること・設定の変更のチャットへの記録・手伝いの口に許されない操作（チャットの送信や退室など）が断られること・どちらからでも停止、が通しで動く | must |
...
REQ-RMT-001〜003・007・030 の「2 台のアプリの間で実際に動くこと」は、シグナリングサーバーとそのサーバーの中継を立てて検証する（末尾）。ここでの検証は、手伝いの申し出と終わりの状態遷移（`src-tauri/src/settings_help.rs`）、手伝いの口の判定と手伝う人に渡さないもの（`src-tauri/src/rpc/help.rs`）、中継の両端と手伝う人の窓の呼び出し（`src-tauri/src/help_link.rs`）、画面の振る舞い（`ui/src/components/SettingsHelp/`・`ui/src/screens/HelperScreen.tsx`）である。
