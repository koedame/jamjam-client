...
同じルームの参加者が、相手の音声の設定を遠隔でしてあげる機能（[ADR-043](./adr/ADR-043-remote-operation-rpc.md)）。

| ID | 要求 | criticality |
|----|------|------------|
| REQ-RMT-001 | 手伝いは、手伝う人が申請し、手伝われる人が許可して初めて始まる。許可すると手伝う人に手伝われる人の音声の設定が届き、ルームのチャットに開始が記録される。断ると何も始まらない。許可の答えは、利用者に見せた申請の相手にだけ効く（その申請が取り下げられて別の人が申請していても、別の人を許可しない） | must |
| REQ-RMT-002 | 手伝う人の変更は 1 件ずつ申請として手伝われる人に届き、許可したものだけが適用される（設定ウィンドウで変えたときと同じ処理で）。申請は 1 件ずつで、答える前に届いた次の申請は捨てる。許可が適用するのは、手伝われる側が番号を付けて利用者に見せた申請であり、手伝う側が付けた番号ではない。出たばかりの申請は少しのあいだ許可を受け付けない（連打で次の申請を許可しない）。断ったもの・答えないまま手伝いが終わったものは適用されない。適用の途中で手伝いが終わり、同じ 2 人が次の手伝いを始めていても、前の変更の結果は次の手伝いに返さない。手伝う人には結果（適用された設定・断られた・適用できなかった理由）が届き、理由は決まった種類（機器が無い・値が使えない・保存できない）で渡す。手伝っていない参加者からの申請・答えは捨てる | must |
| REQ-RMT-003 | 手伝う人も手伝われる人も、いつでも手伝いを止められる（手伝う人は、返事の前の申請も取り下げられる。手伝われる人は、変更の申請に答える画面からも止められる）。止めた・相手が退出した・自分が退出した・接続が切れた・ルームが閉じたとき、手伝いは終わり、待っている変更は捨てられる。止める知らせは、どちら向きの手伝いを止めるかを持つ（2 人が互いに手伝っているとき、片方を止めてももう片方は続く） | must |
| REQ-RMT-004 | 変更が適用されるたびに、ルームのチャットに「（手伝う人）さんが（手伝われる人）さんの（設定の名前）設定を変更しました」と記録される（適用中に手伝いが終わっても記録する）。文は受け取ったアプリが自分の言語で組み立てる。手伝われる人はサーバーが付けた送り主、手伝う人の名前は受け取ったアプリがルームの参加者から引き、ルームにいたことの無い人を名指しした記録は出さない。どちらかが止めたときは終了も記録し、退出・切断で終わったときは退出の記録がそれに代わる | must |
...
| REQ-RMT-006 | 手伝う人には、手伝われる人のデバイス ID を渡さない（シリアル番号が入りうる）。手伝いのあいだだけ使う仮の名前で渡し、変更の申請に付いてきた仮の名前を手伝われる側で元の ID に戻す。仮の名前は入力と出力で別に振り、見せていない仮の名前（別の一覧のものを含む）の申請は、利用者に聞かず返事もせずに捨てる。適用できなかった理由にもエラーの文面（ID やファイルの場所を含みうる）を載せない | must |
...
| REQ-RMT-008 | 2 台のアプリを同じルームに入れたとき、手伝う側の申請・手伝われる側の許可・変更の申請と許可・適用・双方のチャットへの記録・どちらからでも停止、が通しで動く | must |
...
| REQ-RMT-025 | 手伝いの口は、`ui.*`・`debug.*`、チャットの送信とリアクション、退室とルームの移動、接続先と利用状況の送信の設定、ルームの履歴、手伝いの申し出を呼べない。ミュートと音声の設定の変更・読み取りは呼べる | must |
...
REQ-RMT-001〜003 と 007 の「2 台のアプリの間で実際に動くこと」は、シグナリングサーバーを立てて検証する（末尾）。ここでの検証は、手伝いの状態遷移（`src-tauri/src/settings_help.rs`）と画面の振る舞い（`ui/src/components/SettingsHelp/`）である。
...
- `REQ-RMT-008` 2 台のアプリの間で、申請・許可・変更の申請と許可・適用・チャットへの記録・停止が通しで動く
