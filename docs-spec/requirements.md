...
| REQ-UPD-007 | 更新情報の取得先は最新の正式版のもの（GitHub の `releases/latest`）で、ベータ版（pre-release）は配らない | must |
...
REQ-UPD-001 の「新しい版を確かめて入れる」を通しで動かすこと（本物の署名つきの成果物を落として入れ替え、再起動すること）は、OS ごとの実機でしか確かめられない。本要求は、いつ動くか（REQ-UPD-001〜005）、何を信じるか（REQ-UPD-006〜008）、更新情報が成り立つか（REQ-UPD-009〜011）までを検証範囲とする。
