...
| REQ-CON-031 | アプリはルームを作る・入るときに、受け付ける追加の機能（相手あてのメッセージ `peer_message`）を知らせ、参加者の情報から相手のアプリがそれを受け取れるかを知る。機能を知らせない（知らない頃の）アプリ・サーバーの参加者は受け取れないものとして扱い、知らせる機能が無いアプリは項目ごと送らない。相手あてのメッセージは宛先と中身だけを送り、届いたものの送り主はサーバーが付けた値を使う | must |
