...
REQ-CLI-001 の検証はチャットのみで行う（`--chat-only`）。オーディオデバイスの無い環境でも `cargo test` が通る必要があるためである。REQ-CLI-005 は同じ理由で、デバイスの代わりに `--input-tone`（正弦波を送る）と `--output-file`（再生するはずの音をファイルに書く）を使って検証する。実デバイスでの CLI 同士の接続は手動で確認する。
