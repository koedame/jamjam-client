...
| REQ-RMT-027 | デバッグの口から、通話の品質を測る道具が呼べる。入力・送信直前・出力のいずれかを指定時間だけ録音して最大値・RMS・支配的な周波数・途切れを返す（`debug.audio_record`）、送信直前か出力に正弦波を入れる（`debug.audio_tone`）、アプリの画面を PNG で返す（`debug.screenshot`。いまは Linux だけ）、受信した音の欠け（バッファが空だった・間に合わなかった・ロックで待たされた・再同期・パケットの間隔が空いた・受信を運ぶスレッドが止まった・デバイスの読み出しが遅れた／まとまった）を種類ごとに数え、直近の時刻（マイクロ秒）付きで返す（`debug.audio_timing`） | must |
