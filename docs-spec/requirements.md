...
| REQ-RMT-027 | デバッグの口から、通話の品質を測る道具が呼べる。入力・送信直前・出力のいずれかを指定時間だけ録音して最大値・RMS・支配的な周波数・途切れを返す（`debug.audio_record`）、送信直前か出力に正弦波を入れる（`debug.audio_tone`）、アプリの画面を PNG で返す（`debug.screenshot`。いまは Linux だけ） | must |
