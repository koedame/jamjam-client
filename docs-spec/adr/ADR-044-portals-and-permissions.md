...
置き換える範囲のうち、手伝いの作り直し（下記「実装の順序」の 3〜4）が入るまでは、ADR-043 の 3・4 節が実装の姿である。
...
5. **手伝いの作り直し**: 中継の上の口、相手の画面の同期表示、変更ごとの許可の廃止
...
- 画面に残るのは、ダイアログの開閉・入力中のコード・ミキサーのフェーダー・招待リンクが読めなかったときの表示のような、画面だけの状態
